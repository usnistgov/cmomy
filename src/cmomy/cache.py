...
class HashCacheLocator(UserProvidedCacheLocator):  # type: ignore[misc]
    ...
    Use this with ``NUMBA_CACHE_DIR`` and
    ``NUMBA_CACHE_LOCATOR_CLASS="cmomy.cache.HashCacheLocator"``. Cache
    location is identical to default, but with cache key
    ``(NUMBA_CACHE_TOOLS_MAGIC, file-hash)`` where ``file-hash`` is the
    `sha256` of the file.
